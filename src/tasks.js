...
const render = () => {
    const TaskComponent = ({ content, complete, id }) =>
        `<section>
        ${content} <input type="checkbox" name="taskCompleteCheck" data-taskid=${id} ${complete
            ? 'checked'
            : ''}> 
    </section>`;
    ...
    const rendered = tasksStore
        .getState()
        .tasks.filter(task => (state.showComplete ? true : !task.complete))
        .map(TaskComponent)
        .join('');
